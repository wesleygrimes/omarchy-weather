...
function forecastUrl(coordinates) {
  ...
  return (
    ...
    "?latitude=" +
    encodeURIComponent(String(coordinates.latitude)) +
    "&longitude=" +
    encodeURIComponent(String(coordinates.longitude)) +
  ...
  if (currentRaw) {
    current = {
      ...
      feelsLike: formatTemp(
        useImperial ? currentRaw.FeelsLikeF : currentRaw.FeelsLikeC,
        useImperial
      ),
      wind: useImperial ? currentRaw.windspeedMiles + " mph" : currentRaw.windspeedKmph + " km/h",
