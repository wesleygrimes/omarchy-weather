...
equal(weather.parseSavedLocation('{"name": "Malibu", "latitude": 34.02577, "longitude": -118.7804}\n'), {
  name: "Malibu", latitude: 34.02577, longitude: -118.7804
}, "parses name plus coordinates")
equal(weather.parseSavedLocation('{"name": "New York"}'), {
  name: "New York", latitude: null, longitude: null
}, "parses a name-only location")
equal(weather.parseSavedLocation("not json"), weather.emptyLocation(), "treats unparseable location as empty")

equal(weather.locationQuery({ name: "Malibu", latitude: 34.02577, longitude: -118.7804 }), "34.02577,-118.7804", "prefers coordinates")
equal(weather.locationQuery({ name: "New York", latitude: null, longitude: null }), "New%20York", "encodes a name-only location")
equal(weather.locationQuery(weather.emptyLocation()), "", "empty location is auto-detect")
assert.strictEqual(weather.hasCoordinates({ latitude: 36.2, longitude: -79.9 }), true, "saved coordinates count")
assert.strictEqual(weather.hasCoordinates(weather.emptyLocation()), false, "empty location has no coordinates")

equal(weather.parseDetectedPlaceName("Stokesdale, North Carolina, United States"), "Stokesdale", "takes the city from a detected place")
...
equal(
  weather.forecastCoordinates(weather.emptyLocation(), { nearest_area: [{ latitude: "36.237", longitude: "-79.980" }] }),
  { latitude: 36.237, longitude: -79.98 },
...
const conditions = {
  current_condition: [{
    ...
    windspeedKmph: "10", windspeedMiles: "6", humidity: "60", weatherCode: "176"
  }],
  nearest_area: [{
  ...
  weather: [{
    date: "2026-09-19",
    maxtempC: "29", mintempC: "20", maxtempF: "84", mintempF: "67",
    astronomy: [{
      sunrise: "07:06 AM",
      sunset: "07:21 PM",
      moonrise: "03:28 PM",
    ...
    date: "2026-09-20",
    maxtempC: "31", mintempC: "20", maxtempF: "88", mintempF: "68",
    astronomy: [{
      sunrise: "07:07 AM", sunset: "07:19 PM",
      moon_phase: "Waxing Gibbous", moon_illumination: "61",
      moonrise: "04:10 PM", moonset: "12:30 AM"
    }],
    hourly: [{ time: "1200", weatherCode: "176" }]
  }]
}
...
equal(pinned.forecast.map(function(day) { return day.date }), ["2026-09-20", "2026-09-21"], "forecast skips today")
...
const nextDay = weather.buildView({ conditions: conditions, today: "2026-09-20" })
equal(nextDay.sun, { sunrise: "07:07 AM", sunset: "07:19 PM" }, "selects today's sun times from a retained report")
equal(nextDay.moon, {
  phase: "Waxing Gibbous", illumination: "61%", rise: "04:10 PM", set: "12:30 AM"
}, "selects today's moon data from a retained report")

const missingDay = weather.buildView({ conditions: conditions, today: "2026-09-21" })
equal(missingDay.sun, { sunrise: "", sunset: "" }, "does not substitute another day's sun times")
equal(missingDay.moon, { phase: "", illumination: "", rise: null, set: null }, "does not substitute another day's moon data")

const locationOnly = weather.buildView({ location: stokesdale })
equal(locationOnly.location.name, "Stokesdale", "keeps the saved location editable before weather succeeds")
...
assert.notStrictEqual(pinned.current.icon, conditionsOnly.current.icon, "forecast supplies a day/night icon when present")
...
equal(auto.current.icon, pinned.current.icon, "auto-detect still prefers a day/night icon when forecast exists")
